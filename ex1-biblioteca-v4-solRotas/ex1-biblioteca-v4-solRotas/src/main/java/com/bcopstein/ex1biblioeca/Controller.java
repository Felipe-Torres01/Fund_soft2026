...
public class Controller {
    ...
    @CrossOrigin(origins = "*")
    public List<Livro> getLivrosDoAutorAnterior(@PathVariable(value = "ano") int ano) {
        return livros.stream()
                .filter(livro -> livro.getAno() < ano)
                .toList();
    }
    ...
    @CrossOrigin(origins = "*")
    public List<Livro> getLivrosDoAutorAnual(@PathVariable(value = "autor") String autor,
    ...
    @GetMapping("/livro/{id}")
    public Livro getLivroPorId(@PathVariable int id) {
        Livro livroEncontrado = livros.stream() // cria uma variavel e tranf em fluxo
                .filter(livro -> livro.getId() == id)// filtra
                .findAny()// pega o primeiro
                .orElse(null);// se nao achou return null
        return livroEncontrado;
    }

    @DeleteMapping("/livro/{id}")
    public String removeLivroPorId(@PathVariable int id) {
        Livro livroDeletado = livros.stream()
                .filter(livro -> livro.getId() == id)
                .findFirst()
                .orElse(null);

        if (livroDeletado != null) {
            livros.remove(livroDeletado);
